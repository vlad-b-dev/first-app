...
const Pyramid = ({ startPosition, endPosition, duration }) => {
  const ref = useRef();
  const startTime = useRef(Date.now()); // Capture the start time

  const { color, radius, height, radialSegments } = useControls({
    color: "lightblue",
    radius: {
      value: 7,
      min: 1,
...
const MainContent = () => {
  const screenTopRight = [10, 10, 0];
  const screenBottomLeft = [-10, -10, 0];
  const duration = 5;
  ...
  return (
    <>
      ...
      <group position={[0, 0, 0]}>
        <Pyramid
          startPosition={screenTopRight}
          endPosition={screenBottomLeft}
          duration={duration}
        />
...
const MainPageCanvas = () => {
  return (
    <div className="main-page-canvas">
    <Canvas  style={{ width: '100vw', height: '10vh' }}>
      <MainContent />
    </Canvas>
    </div>
  );
